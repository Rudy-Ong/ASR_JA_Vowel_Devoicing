...
inference_r3.py
----------------------
Run transcription with a phone_level3 romaji-rev ConformerASR checkpoint
(train_r3.py — JapaneseRomajiRevTokenizer3, vocab tokenizer_romaji_rev_vocab.json).

Transcripts are phone tokens (``k``, ``ky``, ``sh``, ``ts`` …) with ``cl``
(geminate), ``N`` (moraic ん), ``n`` (na-row), ``pau`` for ideographic commas,
and the devoiced high vowels ``I``/``U`` (voiced counterparts stay ``i``/``u``).

Usage
-----
  # single file
  python inference_r3.py audio.wav

  # multiple files
  python inference_r3.py file1.wav file2.wav file3.wav

  # whole directory
  python inference_r3.py --dir /path/to/wavs/

  # specify checkpoint explicitly
  python inference_r3.py audio.wav --checkpoint models/train_phone3_..._stratified.pt

  # devoiced-vowel confusion matrices (i / u) on test split / full corpus
  python inference_r3.py --eval-scope test

Devoiced vowels are shown in brackets, e.g.  sh [I] t e → I is devoiced.
"""
...
def greedy_decode(model: ConformerASR, mel: torch.Tensor,
                  tokenizer: JapaneseRomajiRevTokenizer3) -> list[int]:
...
def greedy_decode_batch(model: ConformerASR, mel: torch.Tensor,
                        tokenizer: JapaneseRomajiRevTokenizer3,
...
def format_transcript(token_ids: list[int],
                      tokenizer: JapaneseRomajiRevTokenizer3) -> tuple[str, list[str]]:
...
def _default_vocab() -> Path:
    return ROOT / "doc" / "tokenizer_romaji_rev_vocab.json"
...
def _load_transcripts(data_dir: Path) -> dict[str, str]:
    """Read transcript_phone3_rev.txt → {utt_id: space-separated romaji tokens}."""
    transcripts: dict[str, str] = {}
    for line in (data_dir / "transcript_phone3_rev.txt").read_text(encoding="utf-8").splitlines():
        if ":" in line:
...
def evaluate(model: ConformerASR, tokenizer: JapaneseRomajiRevTokenizer3,
    ...
    if scope == "both" and (out_img or out_csv):
        print("Note: --out-img/--out-csv ignored for --eval-scope both; "
              "using fixed cm_vw_phone3_* names.")
...
def main():
    ...
    parser.add_argument("--vocab", type=Path, default=None,
                        help="Path to tokenizer_romaji_rev_vocab.json")
    ...
    parser.add_argument("--data-dir", type=Path,
                        ...
                        help="Dir with transcript_phone3_rev.txt & stratified_manifest.csv")
